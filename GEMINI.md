...
- **AI API**: Google Gemini API (`google-generativeai` SDK)
- **Environment**: `.env` file for API Key management (`python-dotenv`)

## 3. ディレクトリ構成
```text
.
├── app.py              # メインアプリケーション（Streamlitのエントリーポイント）
├── modules/
│   ├── __init__.py
│   ├── gemini_client.py # Gemini APIとの通信クラス（ChatSession管理）
│   └── prompts.py       # システムプロンプト定義ファイル
├── .env                # GOOGLE_API_KEYを格納（git管理外）
...
- クラス `GeminiClient` を作成し、初期化時にAPIキーを設定。
- モデルは `gemini-1.5-flash` を推奨（高速なレスポンス重視）。
- `start_chat` メソッドでシステムプロンプトを設定したセッションを開始する。
- ストリーミング応答 (`stream=True`) に対応できるとなお良い（UX向上のため）。

### 4.3 システムプロンプト (modules/prompts.py)
以下の `SYSTEM_PROMPT` 文字列定数を定義すること。
